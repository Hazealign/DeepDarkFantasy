...
def extractVideo(inputdir, outputdir):
    print subprocess.Popen('ffmpeg -threads 8 -i ' + inputdir + ' -f image2 ' + outputdir + '/%08d.png', shell=True,
...
def deepdream(net, base_img, image_type, iter_n=15, octave_n=4, octave_scale=1.5, end='inception_4c/output', verbose = 1, clip=True, **step_params):
...
def deepdream_guided(net, base_img, image_type, iter_n=15, octave_n=4, octave_scale=1.5, end='inception_4c/output', clip=True, verbose=1, objective_fn=objective_guide, **step_params):
...
def main(input, output, image_type, gpu, model_path, model_name, preview, octaves, octave_scale, iterations, jitter, zoom, stepsize, blend, layers, guide_image, start_frame, end_frame, verbose):
    ...
    if iterations is None: iterations = 15
    if jitter is None: jitter = 40
